...
describe('PV_GENERATOR_FITS:Testing PV generator with fits file.', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Preparation`, () => {
            ...
            let count = 0;
            test(
                `(Step 5): PV Request, halt PV when received 3 pvProgress Stream and check cancel`,
                async () => {
                    let pvProgressPromise = new Promise((resolve) => {
                        msgController.pvProgressStream.subscribe({
                            next: (data) => {
                                count++;
                                console.log(
                                    'request ' + assertItem.openFile.file + ' PV response progress :',
                                    data.progress
                                );
            ...
            test(
                `(Step 6): Request PV Request again`,
                async () => {
