...
let assertItem: AssertItem = {
    ...
    ConcatResponse: ['are not consistent!', 'Duplicate Stokes type found'],
};
            ...
            async () => {
                ...
                try {
                    ConcatStokesResponse = await msgController.loadStokeFiles(
                        assertItem.ConcatReqShape.stokesFiles,
                        assertItem.ConcatReqShape.fileId,
                        assertItem.ConcatReqShape.renderMode
                    );
                } catch (err) {
    ...
    describe(`Case 3: Q & axis-degeneracy Q, duplicated Stokes type`, () => {
        ...
        test(
            ...
            async () => {
                ...
                try {
                    ConcatStokesResponse = await msgController.loadStokeFiles(
                        assertItem.ConcatReqDeplicate.stokesFiles,
                        assertItem.ConcatReqDeplicate.fileId,
                        assertItem.ConcatReqDeplicate.renderMode
                    );
                } catch (err) {
