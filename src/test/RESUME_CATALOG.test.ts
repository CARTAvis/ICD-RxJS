...
describe('RESUME CONTOUR: Test to resume contour lines', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(`Get basepath and modify the assertItem.resumeSession.image.directory`, async () => {
            ...
            assertItem.resumeSession.images[0].directory =
                basepath + '/' + assertItem.resumeSession.images[0].directory;
            assertItem.resumeSession.catalogFiles[0].directory =
                basepath + '/' + assertItem.resumeSession.catalogFiles[0].directory;
        });

        let RegionHistogramData = [];
        test(
            `Resume Image: 2 REGION_HISTOGRAM_DATA & RESUME_SESSION_ACK should arrive within ${resumeTimeout} ms`,
            async () => {
        ...
        describe(`Register another session`, () => {
            ...
            test(
                `Resume Images again: 2 REGION_HISTOGRAM_DATA & RESUME_SESSION_ACK should arrive within ${resumeTimeout} ms`,
                async () => {
                    await msgController.connect(testServerUrl);
                    msgController.histogramStream.pipe(take(2)).subscribe({
                        next: (data) => {
            ...
            test(`Assert the CATALOG_FILTER_RESPONSE.columns.length = ${assertItem.catalogFilterRequest.columnIndices.length}`, async () => {
                ...
                expect(Object.keys(ack[0].columns).length).toEqual(
                    assertItem.catalogFilterRequest.columnIndices.length
                );
            });
        });
