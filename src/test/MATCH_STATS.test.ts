    ...
    describe(`Register a session`, () => {
        ...
        describe(`Prepare images`, () => {
            ...
            for (const [index, region] of assertItem.setRegion.entries()) {
                test(
                    `Should set region ${region.regionId}`,
                    async () => {
            ...
            for (const [fileIdx, file] of assertItem.openFile.entries()) {
                ...
                test(`Assert region_id for file_id: ${file.fileId}`, () => {
                    for (const [regionIdx, region] of assertItem.setRegion.entries()) {
                        expect(
                            RegionStatsData.find(
                                (data) => data[0].fileId == file.fileId && data[0].regionId == region.regionId
                            )[0].statistics.length
                        ).toBeGreaterThan(0);
                    }
                });
            ...
            for (const [regionIdx, region] of assertItem.setRegion.entries()) {
                for (const [statsIdx, statsType] of assertItem.setStatsRequirements[0][
                    ...
                    test(`Assert the ${CARTA.StatsType[statsType]} of region ${region.regionId} for first image equal to that for the second image`, () => {
                        const left = RegionStatsData.find(
                            (data) =>
                                data[0].fileId == assertItem.openFile[0].fileId && data[0].regionId == region.regionId
                        )[0].statistics.find((data) => data.statsType == statsType).value;
                        const right = RegionStatsData.find(
                            (data) =>
                                data[0].fileId == assertItem.openFile[1].fileId && data[0].regionId == region.regionId
                        )[0].statistics.find((data) => data.statsType == statsType).value;
