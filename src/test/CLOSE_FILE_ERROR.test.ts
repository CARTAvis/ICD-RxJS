...
describe('[Case 1] Test for requesting the ICD message of the CLOSED image:', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(
            `(Step 1) OPEN_FILE_ACK and REGION_HISTOGRAM_DATA of fileId = 0 should arrive within ${openFileTimeout} ms `,
            async () => {
        ...
        test(
            `(Step 2) OPEN_FILE_ACK and REGION_HISTOGRAM_DATA of fileId = 1 should arrive within ${openFileTimeout} ms `,
            async () => {
        ...
        test(
            `(Step 4) Test fileId = 0 is still working well: `,
            async () => {
...
describe('[Case 2] Open=>Close=>Open of fileId=0, and then check the backend alive:', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(
            `(Step 4) the backend is still alive`,
            async () => {
                let BackendStatus = await msgController.getFileList(
                    assertItem.filelist.directory,
                    assertItem.filelist.filterMode
