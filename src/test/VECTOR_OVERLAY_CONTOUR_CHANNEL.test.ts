    ...
    setImageChannel: [
        {
            ...
            requiredTiles: {
                fileId: 1,
                tiles: [
                    50339842, 50339841, 50335746, 50335745, 50343938, 50339843, 50343937, 50335747, 50339840, 50331650,
                    50335744, 50331649, 50343939, 50343936, 50331651, 50331648, 50348034, 50339844, 50348033, 50335748,
                    50348035, 50343940, 50348032, 50331652, 50348036,
                ],
                compressionType: CARTA.CompressionType.ZFP,
                compressionQuality: 11,
            },
...
describe('VECTOR_OVERLAY_CONTOUR_CHANNEL: Testing the vector overlay ICD messages with the contours and changing the channel', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`(Case 1) Set vector overlay and contours:`, () => {
            ...
            test(
                `(Step 1) Request Vector Overlay and Response should arrived within ${vectorOverlayTimeout} ms`,
                async () => {
            ...
            test(`(Step 3) Set Contours parameter, Receive the Stream responses and check the correctness:`, async () => {
                ...
                let ContourImageDataResponse: [] = await Stream(
                    CARTA.ContourImageData,
                    assertItem.setContour[0].levels.length
                );
                assertItem.contourImageData.map((data) => {
                    let eachContourImageData: any[] = ContourImageDataResponse.filter(
                        (ResponseData) => ResponseData.contourSets[0].level == data.contourSets[0].level
                    );
                    expect(eachContourImageData[0].progress).toEqual(data.progress);
                    expect(eachContourImageData[0].contourSets[0].decimationFactor).toEqual(
                        data.contourSets[0].decimationFactor
                    );
                    expect(eachContourImageData[0].contourSets[0].level).toEqual(data.contourSets[0].level);
                    expect(eachContourImageData[0].contourSets[0].uncompressedCoordinatesSize).toEqual(
                        data.contourSets[0].uncompressedCoordinatesSize
                    );
        ...
        describe(`(Case 2) Initialization: open the image`, () => {
            test(
                ...
                async () => {
                    ...
                    expect(OpenFileResponse.success).toEqual(true);
                    let RegionHistrogramDataResponse = await Stream(CARTA.RegionHistogramData, 1);
                    expect(RegionHistrogramDataResponse[0].progress).toEqual(
                        assertItem.regionHistogramData[1].progress
                    );
                    expect(RegionHistrogramDataResponse[0].regionId).toEqual(
                        assertItem.regionHistogramData[1].regionId
                    );
                    expect(RegionHistrogramDataResponse[0].fileId).toEqual(assertItem.regionHistogramData[1].fileId);
                    expect(RegionHistrogramDataResponse[0].histograms.binWidth).toBeCloseTo(
                        assertItem.regionHistogramData[1].histograms.binWidth,
                        assertItem.precisionDigits
                    );
                    expect(RegionHistrogramDataResponse[0].histograms.firstBinCenter).toBeCloseTo(
                        assertItem.regionHistogramData[1].histograms.firstBinCenter,
                        assertItem.precisionDigits
                    );
                    expect(RegionHistrogramDataResponse[0].histograms.mean).toBeCloseTo(
                        assertItem.regionHistogramData[1].histograms.mean,
                        assertItem.precisionDigits
                    );
                    expect(RegionHistrogramDataResponse[0].histograms.numBins).toEqual(
                        assertItem.regionHistogramData[1].histograms.numBins
                    );
                    expect(RegionHistrogramDataResponse[0].histograms.stdDev).toBeCloseTo(
                        assertItem.regionHistogramData[1].histograms.stdDev,
                        assertItem.precisionDigits
                    );
                },
                openFileTimeout
        ...
        describe(`(Case 2) Open the second images, and set vector overlay, contour, and change channel:`, () => {
            ...
            test(`(Step 2) Verify the Response (the last VECTOR_OVERLAY_TILE_DATA) correctness`, () => {
                ...
                expect(lastVectorOverlayTileDataResponse.angleTiles[0].layer).toEqual(
                    assertItem.VectorOverlayTileData[1].angleTiles[0].layer
                );
                expect(lastVectorOverlayTileDataResponse.angleTiles[0].width).toEqual(
            ...
            test(`(Step 6: Verify the Response (the last VECTOR_OVERLAY_TILE_DATA of fileid 1 channel 1) correctness)`, () => {
                ...
                expect(lastVectorOverlayTileDataResponse.angleTiles[0].layer).toEqual(
                    assertItem.VectorOverlayTileData[2].angleTiles[0].layer
                );
                expect(lastVectorOverlayTileDataResponse.angleTiles[0].width).toEqual(
            ...
            test(`(Step 7) Verify the Response (REGION_HISTOGRAM_DATA, RASTER_TILE, and RASTER_TILE_SYNC of fileid 1 channel 1) correctness`, () => {
                ...
                expect(RegionHistogramData[0].histograms.binWidth).toBeCloseTo(
                    assertItem.regionHistogramData[2].histograms.binWidth,
                    assertItem.precisionDigits
                );
                expect(RegionHistogramData[0].histograms.firstBinCenter).toBeCloseTo(
                    assertItem.regionHistogramData[2].histograms.firstBinCenter,
                    assertItem.precisionDigits
                );
                expect(RegionHistogramData[0].histograms.mean).toBeCloseTo(
                    assertItem.regionHistogramData[2].histograms.mean,
                    assertItem.precisionDigits
                );
                expect(RegionHistogramData[0].histograms.numBins).toEqual(
                    assertItem.regionHistogramData[2].histograms.numBins
                );
