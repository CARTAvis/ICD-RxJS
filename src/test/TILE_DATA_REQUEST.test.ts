...
let assertItem: AssertItem = {
    ...
    rasterTileData: {
        ...
        assert: [{ lengthTiles: 1 }, { lengthTiles: 1 }, { lengthTiles: 1 }, { lengthTiles: 1 }],
    },
    ...
    rasterTileDataGroup: [
        {
            ...
            assert: [{ lengthTiles: 1 }, { lengthTiles: 1 }, { lengthTiles: 1 }],
        },
        {
            tiles: [
                { x: 1, y: 1, layer: 2 },
...
describe('CHECK_RASTER_TILE_DATA: Testing data values at different layers in RASTER_TILE_DATA', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(
            `RasterTileData * 4 + SpatialProfileData * 1 + RasterTileSync *2 (start & end)?`,
            async () => {
        ...
        assertItem.rasterTileData.tiles.map((tiles, index) => {
            describe(`(Step1-3) Check each RASTER_TILE_DATA`, () => {
                test(`(#${index})RASTER_TILE_DATA.tiles.length = 1 |`, () => {
                    expect(RasterTileDataResponse[index + 1].tiles.length).toEqual(
                        assertItem.rasterTileData.assert[index].lengthTiles
                    );
                });
                ...
                test(`(#${index})RASTER_TILE_DATA.tiles[0].x = ${tiles.x} & RASTER_TILE_DATA.tiles[0].y = ${tiles.y} & RASTER_TILE_DATA.tiles[0].layer = ${tiles.layer}|`, () => {
                    let TempTiles = assertItem.rasterTileData.tiles.filter(
                        (f) =>
                            f.x === RasterTileDataResponse[index + 1].tiles[0].x &&
                            f.y === RasterTileDataResponse[index + 1].tiles[0].y &&
                            f.layer === RasterTileDataResponse[index + 1].tiles[0].layer
                    );
