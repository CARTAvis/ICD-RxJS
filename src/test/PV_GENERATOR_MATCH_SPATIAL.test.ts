...
describe('PV_GENERATOR_MATCH_SPATIAL:Testing PV generator with two spatially matched images.', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Go to "${assertItem.filelist.directory}" folder and open "${assertItem.openFile[0].file}" & "${assertItem.openFile[1].file}"`, () => {
            ...
            test(
                `(step 6): Match the first image to the second image`,
                async () => {
                    ...
                    expect(Response[0].profiles[0].coordinate).toEqual(
                        assertItem.returnPVSpatial.profiles[0].coordinate
                    );
                    ...
                    assertItem.returnPVSpatial.index.map((reference, index) => {
                        expect(Response[0].profiles[0].rawValuesFp32[reference]).toEqual(
                            assertItem.returnPVSpatial.indexValue[index]
                        );
                    });
                },
                Match2ImageTimeout
