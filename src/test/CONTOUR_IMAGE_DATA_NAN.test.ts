            ...
            contourSets: [
                {
                    ...
                    rawCoordinates: new Uint8Array([
                        40, 181, 47, 253, 32, 208, 93, 2, 0, 116, 2, 62, 20, 255, 2, 0, 0, 255, 0, 253, 3, 255, 1, 255,
                        0, 254, 4, 255, 2, 0, 2, 0, 4, 0, 4, 252, 4, 0, 0, 252, 4, 252, 255, 0, 252, 0, 255, 253, 2,
                        252, 13, 0, 85, 116, 96, 98, 228, 1, 199, 157, 65, 36, 196, 198, 68, 133, 192, 76, 25, 12, 69,
                        240, 16, 184, 129, 227, 244, 76, 78, 114, 187, 76, 56, 46,
                    ]),
                },
            ],
            progress: 1,
            ...
            contourSets: [
                {
                    ...
                    rawCoordinates: new Uint8Array([
                        40, 181, 47, 253, 32, 72, 77, 1, 0, 232, 72, 10, 244, 14, 0, 0, 255, 0, 252, 3, 252, 13, 255, 0,
                        244, 9, 247, 7, 249, 13, 247, 3, 255, 255, 255, 10, 0, 0, 0, 4, 0, 224, 32, 51, 57, 201, 237,
                        50, 225, 184,
                    ]),
                },
            ],
            ...
            contourSets: [
                {
                    ...
                    rawCoordinates: new Uint8Array([
                        40, 181, 47, 253, 96, 48, 0, 173, 2, 0, 116, 2, 74, 6, 0, 252, 4, 0, 0, 255, 0, 254, 4, 254, 2,
                        255, 0, 254, 2, 254, 3, 252, 1, 0, 4, 0, 4, 252, 4, 252, 0, 252, 0, 255, 255, 1, 255, 255, 255,
                        0, 253, 17, 0, 69, 17, 24, 130, 192, 12, 11, 17, 190, 119, 247, 218, 12, 2, 38, 127, 144, 50,
                        48, 56, 149, 19, 51, 29, 32, 241, 160, 1, 35, 96, 42, 25, 207, 228, 36, 183, 203, 110, 43, 7,
                        24, 2,
                    ]),
                },
            ],
        ...
        assertItem.contourImageData.map((contour, index) => {
            describe(`SET_CONTOUR_PARAMETERS${index} with SmoothingMode:"${CARTA.SmoothingMode[assertItem.setContour[index].smoothingMode]}"`, () => {
                ...
                test(`contourSets[0].decimationFactor = ${contour.contourSets[0].decimationFactor}`, () => {
                    expect(ContourImageData[0].contourSets[0].decimationFactor).toEqual(
                        contour.contourSets[0].decimationFactor
                    );
                });

                test(`contourSets[0].uncompressedCoordinatesSize = ${contour.contourSets[0].uncompressedCoordinatesSize}`, () => {
                    expect(ContourImageData[0].contourSets[0].uncompressedCoordinatesSize).toEqual(
                        contour.contourSets[0].uncompressedCoordinatesSize
                    );
                });

                test(`number of rawCoordinates = ${contour.contourSets[0].rawCoordinates.length}`, () => {
                    expect(ContourImageData[0].contourSets[0].rawCoordinates.length).toEqual(
                        contour.contourSets[0].rawCoordinates.length
                    );
                });

                test(`Check rawCoordinates value`, () => {
                    for (let idx = 0; idx < contour.contourSets[0].rawCoordinates.length; idx++) {
                        expect(ContourImageData[0].contourSets[0].rawCoordinates[idx]).toEqual(
                            contour.contourSets[0].rawCoordinates[idx]
                        );
                    }
                });
            });
