    ...
    setStatsRequirements: [
        {
            ...
            statsConfigs: [
                {
                    coordinate: 'z',
                    statsTypes: [CARTA.StatsType.NumPixels, CARTA.StatsType.NanCount, CARTA.StatsType.Sum],
                },
            ],
        },
        {
...
describe('RESUME REGION: Test to resume regions', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(`Get basepath and modify the assertItem.resumeSession.image.directory`, async () => {
            ...
            for (let i = 0; i < 2; i++) {
                assertItem.resumeSession.images[i].directory =
                    basepath + '/' + assertItem.resumeSession.images[i].directory;
            }
        });
        ...
        describe(`Register another session`, () => {
            ...
            assertItem.setStatsRequirements.map((stats) => {
                test(
                    `Try to request stats of region ${stats.regionId}`,
                    async () => {
