    ...
    spatialProfileData: [
        {
            ...
            rawValuesPoint: {
                x: {
                    index: [500, 1000, 1500, 2000, 2500],
                    values: [92, 142, 164, 171, 255],
                },
                y: {
                    index: [500, 1000, 1500, 2000, 2500, 3000],
                    values: [255, 56, 244, 246, 1, 255],
                },
            },
        },
        {
        ...
        describe(`set cursor on {${assertItem.setCursor[index].point.x}, ${assertItem.setCursor[index].point.y}}`, () => {
            ...
            test(`Check profiles.rawValuesFp32 value of x coordinate`, () => {
                profileData.rawValuesPoint.x.index.map((f, index) => {
                    expect(SpatialProfileDataTemp.profiles.find((f) => f.coordinate === 'x').rawValuesFp32[f]).toEqual(
                        profileData.rawValuesPoint.x.values[index]
                    );
                });
            });
            ...
            test(`Check profiles.rawValuesFp32 value of y coordinate`, () => {
                profileData.rawValuesPoint.y.index.map((f, index) => {
                    expect(SpatialProfileDataTemp.profiles.find((f) => f.coordinate === 'y').rawValuesFp32[f]).toEqual(
                        profileData.rawValuesPoint.y.values[index]
                    );
                });
            });
        });
