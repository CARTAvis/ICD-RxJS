...
let assertItem: AssertItem = {
    ...
    firstBinsArray: [
        1738900, 1703245, 1634588, 1601625, 1539657, 1481300, 1425994, 1373866, 1348887, 1300739, 1277511, 1255213,
    ],
};

function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms)).then(() => {
        console.log('wait 200ms and send the next setHistogramRequirements!');
    });
}
...
describe('Testing the large image with multi-polygon region and set_histogram_requirement, let the region_histogram_data in queue:', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Go to "${testSubdirectory}" folder and open image "${assertItem.openFile.file}"`, () => {
            ...
            test(
                `(Step 2) Send sequent setHistogramRequirements with different numBins, to simulate the user drag the Number of bins in the setting of Histogram`,
                async () => {
                    let count = 0;
            ...
            test(`(Step 3) Check the receiving RegionHistogramData * 12 `, () => {
                ...
                regionHistogramDataResponse.map((RegionHistogramData, index) => {
                    ...
                    expect(RegionHistogramData.config.bounds.min).toEqual(
                        assertItem.ResponseRegionHistogramData.config.bounds.min
                    );
                    expect(RegionHistogramData.config.bounds.max).toEqual(
                        assertItem.ResponseRegionHistogramData.config.bounds.max
                    );
                    expect(RegionHistogramData.config.fixedBounds).toEqual(
                        assertItem.ResponseRegionHistogramData.config.fixedBounds
                    );
                    expect(RegionHistogramData.config.fixedNumBins).toEqual(
                        assertItem.ResponseRegionHistogramData.config.fixedNumBins
                    );
