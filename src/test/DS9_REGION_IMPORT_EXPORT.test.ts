    ...
    importRegionAck: {
        ...
        regions: {
            ...
            '8': {
                controlPoints: [
                    { x: 320, y: 400 },
                    { x: 369.99951171875, y: 449.99951171875 },
                ],
    ...
    exportRegion: [
        {
            ...
            regionStyles: {
                ...
                '9': {
                    ...
                    lineWidth: 1,
                    name: '',
                    annotationStyle: { textLabel0: 'CARTA REGION TEST' },
                },
                '10': {
                    color: '#2EE6D6',
                    dashList: [],
        ...
        describe(`Go to "${testSubdirectory}" folder and open image "${assertItem.openFile.file}"`, () => {
            ...
            describe(`Import "${assertItem.importRegion.file}"`, () => {
                ...
                test(`Length of IMPORT_REGION_ACK.region = ${Object.keys(assertItem.importRegionAck.regions).length}`, () => {
                    expect(importRegionAckProperties.length).toEqual(
                        Object.keys(assertItem.importRegionAck.regions).length
                    );
                });
                ...
                Object.keys(assertItem.importRegionAck.regions).map((region, index) => {
                    test(`IMPORT_REGION_ACK.region[${index}] = "Id:${region}, Type:${CARTA.RegionType[assertItem.importRegionAck.regions[region].regionType]}"`, () => {
                        ...
                        expect(importRegionAck.regions[importRegionAckProperties[index]].regionType).toEqual(
                            assertItem.importRegionAck.regions[region].regionType
                        );
                        if (assertItem.importRegionAck.regions[region].rotation)
                            expect(importRegionAck.regions[importRegionAckProperties[index]].rotation).toEqual(
                                assertItem.importRegionAck.regions[region].rotation
                            );
                        expect(importRegionAck.regions[importRegionAckProperties[index]].controlPoints).toEqual(
                            assertItem.importRegionAck.regions[region].controlPoints
                        );
                    });
                });
            });
                    ...
                    test(
                        ...
                        async () => {
                            ...
                            exportRegionAck = await msgController.exportRegion(
                                assertItem.exportRegion[idxRegion].directory,
                                assertItem.exportRegion[idxRegion].file,
                                assertItem.exportRegion[idxRegion].type,
                                assertItem.exportRegion[idxRegion].coordType,
                                assertItem.exportRegion[idxRegion].fileId,
                                regionStyle
                            );
                        },
                        exportTimeout
                ...
                describe(`Import "${assertItem.importRegion2[idxRegion].file}" from ${saveSubdirectory}`, () => {
                    ...
                    test(
                        ...
                        async () => {
                            ...
                            importRegionAck = await msgController.importRegion(
                                assertItem.importRegion2[idxRegion].directory,
                                assertItem.importRegion2[idxRegion].file,
                                assertItem.importRegion2[idxRegion].type,
                                assertItem.importRegion2[idxRegion].groupId
                            );
                    ...
                    test(`IMPORT_REGION_ACK.regions[${Region.assertRegionId.index}].region_id = ${Region.assertRegionId.id}`, () => {
                        expect(importRegionAckProperties[importRegionAckProperties.length - 1]).toEqual(
                            String(Region.assertRegionId.id)
                        );
                    });
                });
            });
