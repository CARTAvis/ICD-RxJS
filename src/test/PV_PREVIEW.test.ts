        ...
        describe(`Case ${i + 1}: Test PV preview with "${assertItem.fileOpen[i].file}"`, () => {
            ...
            test(`(Step 3): Set region`, async () => {
                let setRegionAckResponse = await msgController.setRegion(
                    ...
                    assertItem.setRegion[0].previewRegion
                );
            ...
            test(`(Step 4): Set PV request`, async () => {
                ...
                expect(PVresponse.previewData.compressionQuality).toEqual(
                    assertItem.pvResponse.previewData.compressionQuality
                );
                expect(PVresponse.previewData.compressionType).toEqual(
                    assertItem.pvResponse.previewData.compressionType
                );
                ...
                expect(PVresponse.previewData.histogram.binWidth).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogram.binWidth,
                ...
                expect(PVresponse.previewData.histogram.firstBinCenter).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogram.firstBinCenter,
                    assertItem.precisionDigits
                );
                expect(PVresponse.previewData.histogram.mean).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogram.mean,
                    assertItem.precisionDigits
                );
                expect(PVresponse.previewData.histogram.numBins).toEqual(
                    assertItem.pvResponse.previewData.histogram.numBins
                );
                expect(PVresponse.previewData.histogram.stdDev).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogram.stdDev,
                    assertItem.precisionDigits
                );
                expect(PVresponse.previewData.histogramBounds.max).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogramBounds.max,
                    assertItem.precisionDigits
                );
                expect(PVresponse.previewData.histogramBounds.min).toBeCloseTo(
                    assertItem.pvResponse.previewData.histogramBounds.min,
                    assertItem.precisionDigits
                );
                ...
                expect(PVresponse.previewData.imageInfo.axesNumbers.spatialX).toEqual(
                    assertItem.pvResponse.previewData.imageInfo.axesNumbers.spatialX
                );
                expect(PVresponse.previewData.imageInfo.axesNumbers.spectral).toEqual(
                    assertItem.pvResponse.previewData.imageInfo.axesNumbers.spectral
                );
                expect(PVresponse.previewData.imageInfo.axesNumbers.stokes).toEqual(
                    assertItem.pvResponse.previewData.imageInfo.axesNumbers.stokes
                );
                for (i = 0; i < assertItem.pvResponse.previewData.nanEncodings.length; i++) {
                    expect(PVresponse.previewData.nanEncodings[i]).toEqual(
                        assertItem.pvResponse.previewData.nanEncodings[i]
                    );
                }
            });
            ...
            test(`(Step 6): the SET_SPATIAL_REQUIREMENTS of frontend after closing the pv previes`, async () => {
                ...
                expect(SpatialProfileDataResponse[0].regionId).toEqual(
                    assertItem.spatialProfileDataResponse[0].regionId
                );
                expect(SpatialProfileDataResponse[0].profiles[0].end).toEqual(
                    assertItem.spatialProfileDataResponse[0].profiles[0].end
                );
                expect(SpatialProfileDataResponse[0].profiles[0].lineAxis.cdelt).toEqual(
                    assertItem.spatialProfileDataResponse[0].profiles[0].lineAxis.cdelt
                );
                expect(SpatialProfileDataResponse[0].profiles[0].lineAxis.crpix).toEqual(
                    assertItem.spatialProfileDataResponse[0].profiles[0].lineAxis.crpix
                );
                expect(SpatialProfileDataResponse[0].profiles[0].lineAxis.unit).toEqual(
                    assertItem.spatialProfileDataResponse[0].profiles[0].lineAxis.unit
                );
                assertItem.spatialProfileDataRawValueIndex.map((input, index) => {
                    expect(SpatialProfileDataResponse[0].profiles[0].rawValuesFp32[input]).toEqual(
                        assertItem.spatialProfileDataRawValue[index]
                    );
                });
            });
            ...
            test(`(Step 7): Moving SET_REGION with previewRegion = false`, async () => {
                let setRegionAckResponse = await msgController.setRegion(
                    ...
                    assertItem.setRegion[1].previewRegion
                );
                let pVPreviewStream = [];
                ...
                assertItem.pvPreviewStreamImageDataIndex.map((input, index) => {
                    expect(pVPreviewStream[0].histogram.bins[input]).toEqual(
                        assertItem.pvPreviewStreamImageDataValue[index]
                    );
                });
                expect(pVPreviewStream[0].histogram.firstBinCenter).toEqual(
                    assertItem.pvPreviewStream[0].histogram.firstBinCenter
                );
                ...
                expect(pVPreviewStream[0].histogramBounds.max).toBeCloseTo(
                    assertItem.pvPreviewStream[0].histogramBounds.max,
                    assertItem.precisionDigits
                );
                expect(pVPreviewStream[0].histogramBounds.min).toBeCloseTo(
                    assertItem.pvPreviewStream[0].histogramBounds.min,
                    assertItem.precisionDigits
                );
                ...
                expect(SpatialProfileData[0].profiles[0].end).toEqual(
                    assertItem.spatialProfileDataResponse[1].profiles[0].end
                );
                expect(SpatialProfileData[0].profiles[0].lineAxis.cdelt).toEqual(
                    assertItem.spatialProfileDataResponse[1].profiles[0].lineAxis.cdelt
                );
                expect(SpatialProfileData[0].profiles[0].lineAxis.crpix).toEqual(
                    assertItem.spatialProfileDataResponse[1].profiles[0].lineAxis.crpix
                );
                expect(SpatialProfileData[0].profiles[0].lineAxis.unit).toEqual(
                    assertItem.spatialProfileDataResponse[1].profiles[0].lineAxis.unit
                );
            });

            test(`(Step 8): Moving SET_REGION with previewRegion = true`, async () => {
                msgController.spectialSetRegion(
                    assertItem.setRegion[2].fileId,
                    assertItem.setRegion[2].regionId,
                    assertItem.setRegion[2].regionInfo,
                    assertItem.setRegion[2].previewRegion
                );
                let pVPreviewStream = [];
                let pVPreviewStreamPromise = new Promise((resolve) => {
                    msgController.pvPreviewStream.subscribe({
                ...
                expect(pVPreviewStream[0].histogramBounds.max).toBeCloseTo(
                    assertItem.pvPreviewStream[1].histogramBounds.max,
                    assertItem.precisionDigits
                );
                expect(pVPreviewStream[0].histogramBounds.min).toBeCloseTo(
                    assertItem.pvPreviewStream[1].histogramBounds.min,
                    assertItem.precisionDigits
                );
