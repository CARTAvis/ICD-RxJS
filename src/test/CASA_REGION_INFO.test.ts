...
describe('CASA_REGION_INFO: Testing CASA region list and info', () => {
    ...
    describe(`Register a session`, () => {
        ...
        test(`Get basepath`, async () => {
            ...
            assertItem.regionFileInfoRequest[0].directory =
                basepath + '/' + assertItem.regionFileInfoRequest[0].directory;
            assertItem.regionFileInfoRequest[1].directory =
                basepath + '/' + assertItem.regionFileInfoRequest[1].directory;
        });
        ...
        describe(`Go to "${testSubdirectory}" folder and open image "${assertItem.openFile.file}"`, () => {
            ...
            describe(`Go to "${regionSubdirectory}" and send REGION_LIST_REQUEST`, () => {
                ...
                test(
                    `REGION_LIST_RESPONSE should return within ${listTimeout}ms`,
                    async () => {
                        regionListResponse = await msgController.getRegionList(
                            assertItem.regionListRequest.directory,
                            assertItem.regionListRequest.filterMode
                        );
                    },
                    listTimeout
                );
        ...
        assertItem.regionFileInfoResponse.map((fileInfo, idxInfo) => {
            describe(`Read "${assertItem.regionFileInfoRequest[idxInfo].file}"`, () => {
                ...
                test(
                    `REGION_FILE_INFO_RESPONSE should return within ${readTimeout}ms`,
                    async () => {
                        regionFileInfoResponse = await msgController.getRegionFileInfo(
                            assertItem.regionFileInfoRequest[idxInfo].directory,
                            assertItem.regionFileInfoRequest[idxInfo].file
                        );
                    },
                    readTimeout
                );
