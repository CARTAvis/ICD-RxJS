    ...
    describe(`Register a session`, () => {
        ...
        describe(`Preparation`, () => {
            ...
            for (let index = 0; index < assertItem.openFile.length; index++) {
                test(
                    `(Step 1): Should open image ${assertItem.openFile[index].file} as file_id: ${assertItem.openFile[index].fileId}`,
                    async () => {
            ...
            for (const [index, region] of assertItem.setRegion.entries()) {
                test(
                    `(Step 3): Should set regionId of ${index + 1} for file_id: 0`,
                    async () => {
        ...
        describe(`Test if the stats results are equal`, () => {
            ...
            for (const [fileIdx, file] of assertItem.openFile.entries()) {
                test(
                    `Should receive 4 RegionStatsData for file_id: ${file.fileId}`,
                    async () => {
                ...
                test(`Assert region_id for file_id: ${file.fileId}`, () => {
                    for (const [regionIdx, region] of assertItem.setRegion.entries()) {
                        let RegionStatsData = RegionStatsDataArray.find(
                            (data) => data[0]?.fileId == file.fileId && data[0].regionId == regionIdx + 1
                        );
            ...
            for (const [regionIdx, region] of assertItem.setRegion.entries()) {
                for (const [statsIdx, statsType] of assertItem.setStatsRequirements[0][
                    regionIdx
                ].statsConfigs[0].statsTypes.entries()) {
                    test(`Assert the ${CARTA.StatsType[statsType]} of region ${regionIdx + 1} for first image equal to that for the second image`, () => {
                        const left = RegionStatsDataArray.find(
                            (data) =>
                                data[0].fileId == assertItem.openFile[0].fileId && data[0].regionId == regionIdx + 1
                        )[0].statistics.find((data) => data.statsType == statsType).value;
                        const right = RegionStatsDataArray.find(
                            (data) =>
                                data[0].fileId == assertItem.openFile[1].fileId && data[0].regionId == regionIdx + 1
                        )[0].statistics.find((data) => data.statsType == statsType).value;
