            ...
            assertItem.rasterTileDatas.map((rasterTileData: CARTA.IRasterTileData, index: number) => {
                ...
                describe(`Set Image Channel ${JSON.stringify(_channel)}`, () => {
                    ...
                    if (rasterTileData.fileId < 0) {
                        test(
                            `REGION_HISTOGRAM_DATA should not arrive within ${messageReturnTimeout * 0.5} ms.`,
                            async () => {
                        ...
                        test(`REGION_HISTOGRAM_DATA.file_id = ${assertItem.regionHistogramDatas[index].regionId}`, () => {
                            expect(lastRegionHistogramData[0].regionId).toEqual(
                                assertItem.regionHistogramDatas[index].regionId
                            );
                        });

                        test(`REGION_HISTOGRAM_DATA.stokes = ${assertItem.regionHistogramDatas[index].stokes}`, () => {
                            expect(lastRegionHistogramData[0].stokes).toEqual(
                                assertItem.regionHistogramDatas[index].stokes
                            );
                        });
                        ...
                        test(`REGION_HISTOGRAM_DATA.progress = ${assertItem.regionHistogramDatas[index].progress}`, () => {
                            expect(lastRegionHistogramData[0].progress).toEqual(
                                assertItem.regionHistogramDatas[index].progress
                            );
                        });

                        test(`REGION_HISTOGRAM_DATA.histograms.channel = ${assertItem.regionHistogramDatas[index].channel}`, () => {
                            expect(lastRegionHistogramData[0].channel).toEqual(
                                assertItem.regionHistogramDatas[index].channel
                            );
                        });

                        test(
                            `RASTER_IMAGE_DATA should arrive within ${changeChannelTimeout} ms.`,
                            async () => {
