...
let assertItem: AssertItem = {
    ...
    setRegion: [
        ...
        },
        {
            ...
            regionInfo: {
                ...
                controlPoints: [
                    { x: 175.5794044665014, y: 511.6588089330025 },
        ...
        },
        {
            ...
            regionInfo: {
                ...
                controlPoints: [
                    { x: 260.939208984375, y: 346.894500732421 },
                    { x: 400.28278943386033, y: 107.19604796038084 },
                ],
                rotation: 45,
            },
        },
        {
            ...
            regionInfo: {
                ...
                controlPoints: [
                    { x: 157.71339950372214, y: 132.50248138957818 },
                    { x: 100, y: 100 },
                ],
                rotation: 0,
            },
    ...
    importRegionAck: {
        ...
        regions: {
            ...
            '18': {
                controlPoints: [
                    { x: 260.939208984375, y: 346.8945007324219 },
                    { x: 400.28278943386033, y: 107.19604796038084 },
                ],
                regionType: CARTA.RegionType.ANNTEXT,
                rotation: 45,
        ...
        regionStyles: {
            ...
            '18': {
                ...
                annotationStyle: {
                    ...
                    fontSize: 20,
                    fontStyle: 'Normal',
                    textLabel0: 'Double click to edit text',
                    textPosition: 0,
                },
            },
            '19': {
...
describe('Testing set region ICD message to all annotation RegionTypes and export and import for CASA pixel format', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Go to "${testSubdirectory}" folder and open image "${assertItem.openFile.file}"`, () => {
            ...
            test(`(Case 1) Set 10 annotation regions and receive the responses, then check the response:`, async () => {
                for (let index = 0; index < assertItem.setRegion.length; index++) {
                    let setRegionAckResponse = await msgController.setRegion(
                        assertItem.setRegion[index].fileId,
                        assertItem.setRegion[index].regionId,
                        assertItem.setRegion[index].regionInfo
                    );
            ...
            test(
                `(Case 2) Export all annotation RegionTypes as crtf pixel (CASA region) format`,
                async () => {
                    ...
                    exportRegionAck = await msgController.exportRegion(
                        assertItem.exportRegion.directory,
                        assertItem.exportRegion.file,
                        assertItem.exportRegion.type,
                        assertItem.exportRegion.coordType,
                        assertItem.exportRegion.fileId,
                        regionStyle
                    );
            ...
            test(
                `(Case 3) Import the exported region file`,
                async () => {
                    ...
                    importRegionAck = await msgController.importRegion(
                        assertItem.importRegion.directory,
                        assertItem.importRegion.file,
                        assertItem.importRegion.type,
                        assertItem.importRegion.groupId
                    );
                    ...
                    for (let index = 0; index < importRegionAckIndex.length; index++) {
                        ...
                        for (
                            let controlPointsArrayLength = 0;
                            controlPointsArrayLength <
                            importRegionAck.regions[importRegionAckIndex[index]].controlPoints.length;
                            controlPointsArrayLength++
                        ) {
                            expect(
                                importRegionAck.regions[importRegionAckIndex[index]].controlPoints[
                                    controlPointsArrayLength
                                ].x
                            ).toBeCloseTo(
                                assertItem.importRegionAck.regions[importRegionAckIndex[index]].controlPoints[
                                    controlPointsArrayLength
                                ].x,
                        ...
                        if (importRegionAck.regions[importRegionAckIndex[index]].rotation) {
                            expect(importRegionAck.regions[importRegionAckIndex[index]].rotation).toBeCloseTo(
                                assertItem.importRegionAck.regions[importRegionAckIndex[index]].rotation
                            );
                        }

                        //Check the regionType
                        expect(importRegionAck.regions[importRegionAckIndex[index]].regionType).toEqual(
                            assertItem.importRegionAck.regions[importRegionAckIndex[index]].regionType
                        );
                    }

                    //Check each regionStyles
                    for (let index = 0; index < importRegionAckIndex.length; index++) {
                        let regionStylesProperties = Object.keys(
                            importRegionAck.regionStyles[importRegionAckIndex[index]]
                        );
                        //Check dashList
                        expect(importRegionAck.regionStyles[importRegionAckIndex[index]].dashList).toEqual(
                            assertItem.importRegionAck.regionStyles[importRegionAckIndex[index]].dashList
                        );

                        //Check colour
                        expect(importRegionAck.regionStyles[importRegionAckIndex[index]].color).toEqual(
                            assertItem.importRegionAck.regionStyles[importRegionAckIndex[index]].color
                        );

                        //Check lineWidth
                        expect(importRegionAck.regionStyles[importRegionAckIndex[index]].lineWidth).toEqual(
                            assertItem.importRegionAck.regionStyles[importRegionAckIndex[index]].lineWidth
                        );

                        //Check annotationStyle
                        let annotationStyleProperties = Object.keys(
                            importRegionAck.regionStyles[importRegionAckIndex[index]].annotationStyle
                        );
                        annotationStyleProperties.map((property, index) => {
                            let importRegionAckAnnotationArray = new Array(
                                importRegionAck.regionStyles[importRegionAckIndex[index]].annotationStyle
                            );
                            expect(importRegionAckAnnotationArray).toContainEqual(
                                assertItem.importRegionAck.regionStyles[importRegionAckIndex[index]].annotationStyle
                            );
                        });
                    }
                },
