    ...
    openFile: [
        {
            ...
            file: 'S255_IR_sci.spw25.cube.I.pbcor.hdf5',
            hdu: '0',
            fileId: 0,
...
const intensity = [
    // Testing intensity at the (5, 5) of each moment image
    0.86652, 2.2745, 302.11578, 31.72338, 0.91866, 305.16903, 0.18203, 0.88238, 0.09988, 0.9547, 305.16903, 0.67406,
    251.49975,
];
...
describe('PERF_MOMENT_GENERATOR', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Initialization: open the image`, () => {
            test(
                `(Step 1)"${assertItem.openFile[0].file}" OPEN_FILE_ACK and REGION_HISTOGRAM_DATA should arrive within ${openFileTimeout} ms`,
                async () => {
            ...
            test(
                `(Step 2)"${assertItem.openFile[0].file}" Set SET_SPECTRAL_REQUIREMENTS, the responses should arrive within ${setSpectralReqTimeout} ms`,
                async () => {
            ...
            describe(`Moment generator`, () => {
                test(
                    `(Step 3)"${assertItem.openFile[0].file}": Receive a series of moment progress within ${momentTimeout}ms`,
                    async () => {
