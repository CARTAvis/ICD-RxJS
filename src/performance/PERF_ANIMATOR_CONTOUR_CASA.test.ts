...
let assertItem: AssertItem = {
    ...
    openFile: {
        ...
        file: 'cube_B_06400_z00100.image',
        hdu: '',
        fileId: 0,
    ...
    startAnimation: [
        ...
        {
            fileId: 0,
            startFrame: { channel: 40, stokes: 0 },
            firstFrame: { channel: 0, stokes: 0 },
            lastFrame: { channel: 44, stokes: 0 },
            deltaFrame: { channel: -1, stokes: 0 },
            requiredTiles: {
...
describe('ANIMATOR_CONTOUR: Testing animation playback with contour lines', () => {
    ...
    describe(`Register a session`, () => {
        ...
        describe(`Go to "${assertItem.filelist.directory}" folder`, () => {
            ...
            test(
                `"${assertItem.openFile.file}" OPEN_FILE_ACK and REGION_HISTOGRAM_DATA should arrive within ${openFileTimeout} ms`,
                async () => {
            ...
            test(
                ...
                async () => {
                    ...
                    let ContourImageDataResponse = await Stream(
                        ...
                        assertItem.setContour.levels.length
                    );
                },
                readFileTimeout
            );
        ...
        describe(`(Case 1):Play some channels forwardly`, () => {
            test(`Preparation`, async () => {
                ...
                let RasterTileDataResponse = await Stream(
                    CARTA.RasterTileData,
                    assertItem.setImageChannel[0].requiredTiles.tiles.length + 2
                );
            });
            ...
            test(`Received RasterTileData channels should be in sequence`, async () => {
                ...
                RasterTileSequence.map((id, index) => {
                    let channelId =
                        index +
                        assertItem.startAnimation[0].startFrame.channel +
                        assertItem.startAnimation[0].deltaFrame.channel -
                        1;
    ...
    describe(`Register a session`, () => {
        ...
        describe(`(Case 2) Play some channels backwardly`, () => {
            test(`Preparation`, async () => {
                ...
                let RasterTileDataResponse = await Stream(
                    CARTA.RasterTileData,
                    assertItem.setImageChannel[1].requiredTiles.tiles.length + 2
                );
            });
            ...
            test(
                ...
                async () => {
                    ...
                    for (
                        let i = assertItem.startAnimation[1].startFrame.channel;
                        i > assertItem.stopAnimation[1].endFrame.channel - 1;
                        i--
                    ) {
                        msgController.addRequiredTiles(assertItem.addTilesReq);
                        let resRegionHistogramData = msgController.histogramStream.pipe(take(1)).subscribe({
            ...
            test(`Received ContourData channels should be in sequence`, async () => {
                let index = 1;
                for (
                    let i = assertItem.startAnimation[1].startFrame.channel - 1;
                    i > assertItem.stopAnimation[1].endFrame.channel - 2;
                    i--
                ) {
