pipeline {
                ...
                stage('3 macOS 11') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "macos11-backend"
                        }
                    }
                    post {
                        failure {
                ...
                stage('4 macOS 12') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "macos12-backend"
                        }
                    }
                    post {
                        failure {
                ...
                stage('5 macOS 13') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "macos13-backend"
                        }
                    }
                    post {
                        failure {
                ...
                stage('6 RHEL7') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "rhel7-backend"
                        }
                    }
                    post {
                        failure {
                ...
                stage('7 RHEL8') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "rhel8-backend"
                        }
                    }
                    post {
                        failure {
                ...
                stage('8 RHEL9') {
                    ...
                    steps {
                        catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                            ...
                            stash includes: "carta-backend/build/carta_backend", name: "rhel9-backend"
                        }
                    }
                    post {
                        failure {
    ...
    post {
        success {
            slackSend color: 'good', message: "ICD-RxJS - Success - carta-backend: ${BACKENDBRANCH} - ICD-RxJS: ${BRANCH_NAME} <${env.RUN_DISPLAY_URL}|${COMMIT_ID_SHORT}>";
            ...
            -H "Authorization: token $TOKEN" https://api.github.com/repos/CARTAvis/ICD-RxJS/statuses/${COMMIT_ID_LONG} \
            -d "{\\"state\\":\\"success\\",\\"target_url\\":\\"${env.RUN_DISPLAY_URL}\\",\\"description\\":\\"Success\\",\\"context\\":\\"ICD test suite\\"}" """)
            }

        }
        unstable {
            slackSend color: 'warning', message: "ICD-RxJS - Unstable - carta-backend: ${BACKENDBRANCH} - ICD-RxJS: ${BRANCH_NAME} <${env.RUN_DISPLAY_URL}|${COMMIT_ID_SHORT}>";
            withCredentials([string(credentialsId: 'acdc-jenkins-token', variable: 'TOKEN')]) {
            sh("""curl -X POST -H "Accept: application/vnd.github+json" \
            -H "Authorization: token $TOKEN" https://api.github.com/repos/CARTAvis/ICD-RxJS/statuses/${COMMIT_ID_LONG} \
...
def region_manipulation() {
    script {
        ...
        retry(3) {
            ...
            sh "pgrep carta_backend"
            sh "CI=true npm test src/test/CASA_REGION_INFO.test.ts # test 1 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/CASA_REGION_IMPORT_INTERNAL.test.ts # test 2 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/CASA_REGION_IMPORT_EXPORT.test.ts # test 3 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/CASA_REGION_IMPORT_EXCEPTION.test.ts # test 4 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/CASA_REGION_EXPORT.test.ts # test 5 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/DS9_REGION_EXPORT.test.ts # test 6 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/DS9_REGION_IMPORT_EXCEPTION.test.ts # test 7 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/DS9_REGION_IMPORT_EXPORT.test.ts # test 8 of 9"
            sh "sleep 3 && pgrep carta_backend"
            sh "CI=true npm test src/test/SET_REGION_ANNOTATION_EXPORT_IMPORT_CASA_PIXEL.test.ts # test 9 of 9"
            sh "pgrep carta_backend"
        }
    }
}
